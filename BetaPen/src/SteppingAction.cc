...
//collect energy ect step by step
 ...
 if(volume == fDetector->GetScint()){ 
	if(ParticleName=="e-"||ParticleName=="e+"||ParticleName=="gamma"||ParticleName=="alpha"){
	fEventAction->AddEdep(edep);
	if(aStep->IsLastStepInVolume()){
	//std::cout<<"LastStep\n";
	G4ThreeVector sneak = track->GetPosition();
	G4double sneake = track->GetKineticEnergy();
	fEventAction->SneakyTrack(sneake,sneak);
}
	}
	if(ParticleName!="e-"&&ParticleName!="e+"&& ParticleName!="gamma"){
	//std::cout<<ParticleName.c_str()<<"\n";
	//track->SetTrackStatus(fStopAndKill);
...
//if(track->GetCreatorProcess()->GetProcessName()=="RadioactiveDecayBase"){
	G4double tot = track->GetVertexKineticEnergy();

fEventAction->AddInitial(tot);}
 ...
 if (secondaries->size()>0) {
   for(unsigned int i=0; i<secondaries->size(); ++i) {
		...
		if (secondaries->at(i)->GetDynamicParticle()->GetParticleDefinition()->GetParticleName()== "e-"){
		if (secondaries->at(i)->GetCreatorProcess()->GetProcessName() == "RadioactiveDecayBase"){
			fEventAction->AddTOT(secondaries->at(i)->GetKineticEnergy());}}
