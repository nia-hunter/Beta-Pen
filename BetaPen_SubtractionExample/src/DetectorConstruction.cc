...
//++++++++++++++++++++++++++++++++++++++++++++
#include "G4SubtractionSolid.hh" // Add this line to make subtraction solids
//++++++++++++++++++++++++++++++++++++++++++++
...
//G4VPhysicalVolume* Scint_phys =
      fScint = new G4PVPlacement(0,G4ThreeVector(0,0,1.75*mm),Guide_log,"Scint",
                        expHall_log,false,0);
...
G4Box* CookieCutter = new G4Box("Cutter",0.31,0.31,1.76); // make a cutter slightly bigger than the crystal to not cause surface problems
G4Tubs* EpoxyFill = new G4Tubs("Dummy",0.*mm, 0.912*mm,1.75*mm,0*deg,360*deg); //make a solid epoxy block that doesn't quite touch the sides
G4SubtractionSolid* SpaceFiller = new G4SubtractionSolid("EpoxyFiller",EpoxyFill,CookieCutter,0,G4ThreeVector(0,0,0)); //make the subtraction
G4LogicalVolume* Filler_log = new G4LogicalVolume(SpaceFiller,Al,"EpoxyFiller",0,0,0); // make the logical volume--left Al needs to be changed
G4VPhysicalVolume* Filler_phys = new G4PVPlacement(0,G4ThreeVector(0,0,1.75*mm),Filler_log,"EpoxyFiller",expHall_log,false,0); //Place in the world
